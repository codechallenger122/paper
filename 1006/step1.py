...
device = "mgc_fft_1"
#device = "mgc_matrix_mult_1"

pwd1 = "C:\/Users\/김화평\/Desktop\/workSpace\/benchMark\/"
#pwd1 = "C:\/Users\/HPK\/Desktop\/paper\/benchMark\/"

""" dcell 의 width, height 은 (2, 1) 이고, 
    region 에는 dcell 이 10 x 40 = 400 개가 들어간다.
    region 에는 95% 만 채워진다. """
...
def pushCellToRegion(idx, regionUcellXea, regionUcellYea, numOfRegionRow, numOfRegionCol):
    if(globCellName[idx] == "g540091" or globCellName[idx] == "g540182"): print(idx, globCellName[idx])
    ...
    width = globCellW[idx]
    fLogStep2.write("="*50+"\n")
    fLogStep2.write(" Cell ("+str(idx)+") + number"+ globCellName[idx] + "with (x', y') = ("+str(xLL)+", "+str(yLL)+") with (w, h) = ("+str(width)+","+str(height)+") is picked\n")
...
print("unitHeight : ", unitHeight, "um")
print("unitWidth : ", unitWidth, "um")
print("blockWidth : ", blockWidth, "um")
print("blockHeight : ", blockHeight, "um")
numOfRow = int(blockHeight/unitHeight)
numOfCol = int(blockWidth/unitWidth)

regionUcellYea = regionDcellYea * dcellHeight   # 예시.   10 = 10 * 1
regionUcellXea = regionDcellXea * dcellWidth    # 예시.   80 = 40 * 2

numOfRegionRow = int(numOfRow/regionUcellYea)
lastRegionUcellYea = regionUcellYea + numOfRow%regionUcellYea
numOfRegionCol = int(numOfCol/regionUcellXea)   # 예시.   12개 = 1000/80
lastRegionUcellXea = regionUcellXea + numOfCol%regionUcellXea

writePrint("="*100, fLogStep1)
writePrint("unitHeight = "+ str(unitHeight)+ "um", fLogStep1)
writePrint("unitWidth  = "+ str(unitWidth)+ "um",  fLogStep1)
writePrint("blockHeight = "+ str(blockHeight)+ "um", fLogStep1)
writePrint("blockWidth  = "+ str(blockWidth)+ "um",  fLogStep1)
...
for cell in sortedCellList:
    ...
    fSortedCell.write("{:>8}".format(idx) + "th Cell : " + 
                      "X = {:<10}".format(cell[1]) +
                      "Y = {:<10}".format(cell[2]) +
                      "width  = {:<10}".format(cell[3]) +
                      "height  = {:<10}".format(cell[4]) +        
                      "num of width = {:<10}".format(ColA) +
                      "num of height = {:<10}".format(RowA) +
                      "\n")
    idx+=1
fSortedCell.close()
...
def printCapacity():
    print("numOfRegionRow :", numOfRegionRow, "\nnumOfRegionCol :", numOfRegionCol)
    for row in range(numOfRegionRow):
        if row != numOfRegionRow-1: capY = regionUcellYea
        else: capY = lastRegionUcellYea    
        for col in range(0, numOfRegionCol):
            if col != numOfRegionCol-1: capX = regionUcellXea
            else: capX = lastRegionUcellXea
            print("(row, col) : (", row, ", ", col, ")",  capY, ", ", capX, ", ", capaFactor, ", ", regionCap[row][col])
...
for i in range(0, numOfRegionRow):
    for j in range(0, numOfRegionCol):
        ...
        for x,y,type,rowNumb,colNumb,rowFull,colFull in regionCell[i][j]:
            ...
            str(rowNumb+1) + "," + str(colNumb+1) + "," + str(rowFull) + "," + str(colFull) +","+ globCellName[type] + "\n")
