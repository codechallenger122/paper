...
def addPatchToFig(fig, data):
    deltaY = 1/regionDcellYea
    deltaX = 1/regionDcellXea

    cellId = str(data[0]) + "\n(" + str(data[3]) + ", " + str(data[4]) + ")\n/(" + str(data[5]) + ", " + str(data[6]) + ")"
    row, col = data[1], data[2]
    left, width = deltaX*(col), deltaX
    bottom, height = deltaY*(row), deltaY
...
with open(regionCellFile, "r") as f:
    ...
    while True:
        ...
        elif len(args) == 8: # x좌표, y좌표, sorting 된 number = ID, rowNumb, colNumb, globCellName
...
for row in range(numOfRegionRow):
    for col in range(numOfRegionCol):
        if row == numOfRegionRow -1 : inRegionRow = int(lastRegionUcellYea/dcellHeight)
        else : inRegionRow = int(regionUcellYea/dcellHeight)

        if col == numOfRegionCol -1 : inRegionCol = int(lastRegionUcellXea/dcellWidth)
        else : inRegionCol = int(regionUcellXea/dcellWidth)
        ...
        #print(row, ", ", col, ", ", inRegionRow, ", ", inRegionCol,", position 수 : ", numOfPos, " 현재 할당된 수: ", regionNumbDict[row][col] )
        if delta != 0:
            for i in range(delta):
                regionCellDict[row][col].append([-1,-1,-1,-1,-1,-1,-1]) # dummy cell 추가.
                regionNumbDict[row][col] +=1
        #print(row, ", ", col, ", ", inRegionRow, ", ", inRegionCol,", position 수 : ", numOfPos, " 현재 할당된 수: ", regionNumbDict[row][col] )
...
def makeHungrianGraph(row, col):
    cost = []
    cidx = -1
    print(len(regionCellDict[row][col]), len(regionPosDict[row][col]))
...
def saveHungResult(cellLists, row, col):
...
def runFinal(row, col):
    ...
    saveHungResult(cellLists, row, col)
    ...
    del regionNumbDict[row][col]
    del regionCellDict[row][col]
    del regionPosDict[row][col]
    del posXYDict[row][col]
    del rowMaxColMaxInfo[row][col]
...
# Test Code
#cellLists, totalCost = runHungary(10, 21) # cellList = [cellId, rowPos, colPos, rowNumb, colNumb, rowMax, colMax
#saveHungResult(cellLists, 10, 21)
#showDistribute(cellLists)
#print(posXYDict[10][21])

# last code
def doTest():
    t1 = time()
    for row in range(5, 6):
        for col in range(5, 6): # 21 22
...
def doFinal():
    ...
    for row in range(numOfRegionRow):
        ...
        for col in range(numOfRegionCol):
            print("row: ", row, " col: ", col, "processing...")
            runFinal(row, col)
    print(datetime.timedelta(seconds=sec), "is consumed.")
#doFinal()
