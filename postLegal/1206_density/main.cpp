...
int main()
{
    ...
    // bin calculate <- new 22.12.06
    vector<int> blockage;
    cout << calcPenalty(blockAreaMap, blockX, blockY, siteX, siteY, binLength, r_target, blockage) << endl;
    ...
    runAlgorithm_try(dataHPWL, cellsInNet, netName, cellInfo, siteX, siteY, blockCellMap, validCell, netsInCell, 
        logCalcCost, marginA, marginB, maxDisp, HPWLArch, netIter, blockAreaMap, numOfRow, numOfCol, blockX, blockY, binLength, blockage);
    finish = clock();
    cout << calcPenalty(blockAreaMap, blockX, blockY, siteX, siteY, binLength, r_target, blockage) << endl;
