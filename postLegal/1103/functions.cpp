...
tuple<float, int> calculateSwapCost(int nId, int cId, int scId, vector<vector<float>>& cellInfo, vector<vector<int>>& netsInCell, 
    ...
    cout << " cell : " << cId <<  " 's displacment changes from " << displace_cId_before  << " to " << displace_cId_after  << endl;
    ...
    for (int i = 0; i < netsInCell[scId].size(); i++) {
        ...
        //HPWL_before += getHPWL(net, cellsInNet, cellInfo);
        HPWL_before += readHPWL(net, HPWLArch);
        cout << "before! < " << getHPWL(net, cellsInNet, cellInfo) << " - after > " << readHPWL(net, HPWLArch);
    }
    ...
    for (int i = 0; i < netsInCell[cId].size(); i++) {
        ...
        HPWL_after += readModifiedHPWL(net, HPWLArch, cId, scId, cellInfo);
        cout << "before.. " << getHPWLWithNewLeg(net, cellsInNet, cellInfo, cId, scId) << " after :" << readModifiedHPWL(net, HPWLArch, cId, scId, cellInfo);
    }
    for (int i = 0; i < netsInCell[scId].size(); i++) {
        int net = netsInCell[scId][i];
        //HPWL_after += getHPWLWithNewLeg(net, cellsInNet, cellInfo, cId, scId);
...
void runAlgorithm(vector<tuple<int, float>>& dataHPWL, vector<vector<int>>& cellsInNet, vector<string>& netName, 
    ...
    vector<bool>& validCell, vector<vector<int>>& netsInCell, string logCalcCost, float marginA, float marginB, float maxDisp, vector<DataHPWL> &HPWLArch) {
    //float marginA = 1.2;
    //float marginB = 1.2;
    ...
    for (auto i = 0; i < 3000; i++) {
        ...
        while (!queue.empty()) {
            ...
            for (int i = 0; i < candidateCells.size(); i++) {
                ...
                cout << i << " 번째 candidate cell," << scId << "cost 계산 완료" << endl;
            ...
            if (cost.empty()) continue;
                ...
                updataHPWL(cId, scId_picked, cellsInNet, netsInCell, cellInfo, HPWLArch);
...
float readModifiedHPWL(int net, vector<DataHPWL>& HPWLArch, int delCell, int addCell, vector<vector<float>> & cellInfo) {
...
void updataHPWL(int cId, int scId_picked, vector<vector<int>>& cellsInNet, vector<vector<int>>& netsInCell, vector<vector<float>>& cellInfo, vector<DataHPWL>& HPWLArch) {
