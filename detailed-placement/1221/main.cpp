...
float marginA = 1.2; // getWindowCell
float marginB = 1.2; // calculateCost - distance 최대값을 얼마까지 감수할 것인지? 15.4 초
float netIter;

string mainPath = "C:\/Users\/김화평\/Desktop\/workSpace\/ispd_2015_benchMarks\/mgc_des_perf_1\/res\/";
float blockX = 445, siteX = 0.2; float blockY = 445, siteY = 2; float maxDisp = 50;
float marginA = 1.5; // getWindowCell
float marginB = 1.5; // calculateCost - distance 최대값을 얼마까지 감수할 것인지? 15.4 초
float netIter;
...
float marginA = 1.4; // getWindowCell
float marginB = 1.0; // calculateCost - distance 최대값을 얼마까지 감수할 것인지? 15.4 초
string mainPath = "C:\/Users\/김화평\/Desktop\/workSpace\/ispd_2015_benchMarks\/mgc_des_perf_b\/res\/";
...
float marginA = 1.45; // getWindowCell
float marginB = 1.1; // calculateCost - distance 최대값을 얼마까지 감수할 것인지? 15.4 초
string mainPath = "C:\/Users\/김화평\/Desktop\/workSpace\/ispd_2015_benchMarks\/mgc_fft_b\/res\/";
float blockX = 800, siteX = 0.2; float blockY = 800, siteY = 2; float maxDisp = 500;
float marginA = 1.45; // getWindowCell
float marginB = 1; // calculateCost - distance 최대값을 얼마까지 감수할 것인지? 15.4 초
...
string mainPath = "C:\/Users\/김화평\/Desktop\/workSpace\/ispd_2015_benchMarks\/mgc_des_perf_1\/res\/";
float blockX = 445, siteX = 0.2; float blockY = 445, siteY = 2; float maxDisp = 50;
float marginA = 1.6; // getWindowCell
float marginB = 1.6; // calculateCost - distance 최대값을 얼마까지 감수할 것인지? 15.4 초
int netIter = 3000;
...
string resFileName = "detailed_placement_result.txt";
...
int main()
{
    ...
    //netIter = 5000;
    start = clock();
    runAlgorithm_try(dataHPWL, cellsInNet, netName, cellInfo, siteX, siteY, blockCellMap, validCell, netsInCell, logCalcCost, marginA, marginB, maxDisp, HPWLArch, netIter, blockAreaMap, numOfRow, numOfCol);
    finish = clock();

    double duration = (double)(finish - start) / CLOCKS_PER_SEC;
    cout << duration << "초" << endl;
    checkOverlap(cellInfo, siteX, siteY, blockX, blockY);
    writeOutput(resFileName, cellInfo, cellName);
}
